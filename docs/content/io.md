...
| `WriteRigPlyFile(ply_file, reconstruction, sensor_color, min_num_observations_per_point=2, …)` | `bool` | Rig-aware PLY: tracks + capture body centers + sensor centers + densely sampled **trajectory** and **intra-capture baseline** polylines (MeshLab / Open3D). Optional `capture_color`, `trajectory_color`, `baseline_color`, `edge_samples`. |
