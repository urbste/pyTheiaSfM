...
EstimateAbsolutePoseWithKnownOrientationWrapper(
  ...
  const bool success =
      EstimateAbsolutePoseWithKnownOrientation(ransac_params,
                                               ransac_type,
                                               camera_orientation,
...
std::tuple<bool, Plane, RansacSummary> EstimateDominantPlaneFromPointsWrapper(
  ...
  Plane plane;
  RansacSummary ransac_summary;
  const bool success = EstimateDominantPlaneFromPoints(
      ransac_params, ransac_type, points, &plane, &ransac_summary);
...
std::tuple<bool, Eigen::Matrix3d, RansacSummary> EstimateEssentialMatrixWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success = EstimateEssentialMatrix(ransac_params,
                                               ransac_type,
                                               normalized_correspondences,
                                               &essential_matrix,
                                               &ransac_summary);
...
EstimateFundamentalMatrixWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success = EstimateFundamentalMatrix(ransac_params,
                                                 ransac_type,
                                                 correspondences,
                                                 &fundamental_matrix,
                                                 &ransac_summary);
...
std::tuple<bool, Eigen::Matrix3d, RansacSummary> EstimateHomographyWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success = EstimateHomography(ransac_params,
                                          ransac_type,
                                          correspondences,
                                          &homography,
                                          &ransac_summary);
...
EstimateRadialHomographyMatrixWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success =
      EstimateRadialHomographyMatrix(ransac_params,
                                     ransac_type,
                                     normalized_correspondences,
                                     &result,
                                     &ransac_summary);
...
std::tuple<bool, RelativePose, RansacSummary> EstimateRelativePoseWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success = EstimateRelativePose(ransac_params,
                                            ransac_type,
                                            normalized_correspondences,
                                            &relative_pose,
                                            &ransac_summary);
...
EstimateRelativePoseWithKnownOrientationWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success =
      EstimateRelativePoseWithKnownOrientation(ransac_params,
                                               ransac_type,
...
EstimateRigidTransformation2D3DWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success =
      EstimateRigidTransformation2D3D(ransac_params,
                                      ransac_type,
                                      correspondences,
...
EstimateSimilarityTransformation2D3DWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success =
      EstimateSimilarityTransformation2D3D(ransac_params,
                                           ransac_type,
                                           correspondences,
...
std::tuple<bool, Eigen::Vector4d, RansacSummary> EstimateTriangulationWrapper(
  ...
  const bool success = EstimateTriangulation(
      ransac_params, cameras, features_, &triangulated_point, &summary);
...
EstimateUncalibratedAbsolutePoseWrapper(
  ...
  RansacSummary ransac_summary;
  const bool success =
      EstimateUncalibratedAbsolutePose(ransac_params,
                                       ransac_type,
                                       normalized_correspondences,
                                       &absolute_pose,
                                       &ransac_summary);
