...
std::tuple<bool, Vector4d> TriangulateNViewSVDWrapper(
    const std::vector<Matrix3x4d>& poses, const std::vector<Vector2d>& points) {
...
std::tuple<bool, Vector4d> TriangulateNViewWrapper(
    const std::vector<Matrix3x4d>& poses, const std::vector<Vector2d>& points) {
  Vector4d triangulated_point;
...
std::tuple<bool, Vector4d> TriangulateMidpointWrapper(
    const std::vector<Vector3d>& origins,
    const std::vector<Vector3d>& ray_directions) {
  Vector4d triangulated_point;
  const bool success =
      TriangulateMidpoint(origins, ray_directions, &triangulated_point);
  return std::make_tuple(success, triangulated_point);
}
...
std::tuple<bool, Vector4d> TriangulateDLTWrapper(const Matrix3x4d& pose1,
  ...
  Vector4d triangulated_point;
  const bool success =
      TriangulateDLT(pose1, pose2, point1, point2, &triangulated_point);
  return std::make_tuple(success, triangulated_point);
}
