...
// PoseLib license text. Math kept as close as possible to upstream to keep
// future diffs against PoseLib reviewable; only the surrounding types
// (namespace, output-only-essential-matrix) were adapted.
//
...
void compute_trace_constraints(const Eigen::Matrix<double, 4, 9>& N,
                               Eigen::Matrix<double, 10, 20>* coeffs_ptr) {
  Eigen::Matrix<double, 10, 20>& coeffs = *coeffs_ptr;
...
int FivePointRelativePoseSturm(const std::vector<Eigen::Vector3d>& x1h,
  ...
  CHECK_EQ(x1h.size(), 5) << "FivePointRelativePoseSturm is a strictly "
                             "minimal solver and requires exactly 5 point "
                             "correspondences.";
  CHECK_EQ(x2h.size(), 5);
  ...
  for (int i = 0; i < 5; ++i) {
    epipolar_constraints.col(i) << x1h[i](0) * x2h[i], x1h[i](1) * x2h[i],
        x1h[i](2) * x2h[i];
  }
  const Eigen::Matrix<double, 9, 9> Q =
      epipolar_constraints.fullPivHouseholderQr().matrixQ();
  const Eigen::Matrix<double, 4, 9> N = Q.rightCols(4).transpose();
  ...
  coeffs.block<10, 10>(0, 10) =
      coeffs.block<10, 10>(0, 0).partialPivLu().solve(
          coeffs.block<10, 10>(0, 10));
  ...
  double roots[10];
  const int n_sols = theia::sturm::bisect_sturm<10>(c, roots);
