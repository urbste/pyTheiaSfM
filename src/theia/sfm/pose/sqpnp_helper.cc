...
double AverageSquaredProjectionError(int sol_idx, const SQPSolution& solution,
  const std::vector<Eigen::Vector2d>& projections, const std::vector<Eigen::Vector3d>& points)  {
    double avg = 0.0;
    ...
    {
      double Xc     =       r[0]*points.at(i).vector[0] + r[1]*points.at(i).vector[1] + r[2]*points.at(i).vector[2] + t[0], 
            Yc     =       r[3]*points.at(i).vector[0] + r[4]*points.at(i).vector[1] + r[5]*points.at(i).vector[2] + t[1], 
            inv_Zc = 1 / ( r[6]*points.at(i).vector[0] + r[7]*points.at(i).vector[1] + r[8]*points.at(i).vector[2] + t[2] );
      avg += ( Xc*inv_Zc - projections.at(i).vector[0] ) * ( Xc*inv_Zc - projections.at(i).vector[0] ) +
            ( Yc*inv_Zc - projections.at(i).vector[1] ) * ( Yc*inv_Zc - projections.at(i).vector[1] );
    }
...
std::vector<double> AverageSquaredProjectionErrors(const std::vector<SQPSolution>& solutions,
  const std::vector<Eigen::Vector2d>& projections, const std::vector<Eigen::Vector3d>& points) {
    std::vector<double> avg_errors;
    avg_errors.reserve(solutions.size());
    for (size_t i = 0; i < solutions.size(); i++)
    {
        avg_errors.emplace_back( AverageSquaredProjectionError(i, solutions[i], projections, points) );
    }
    return avg_errors;
  }
...
void HandleSolution(const Matrix99& Omega,
  ...
  SQPSolution& solution, double& min_sq_error)
  {
	...
	else // look for a solution that's almost equal to this
	{
	  ...
	  {
	    if ( ( solutions[i].r_hat - solution.r_hat ).squaredNorm() < parameters_.equal_vectors_squared_diff )
	    {
...
inline void NearestRotationMatrix_SVD(const Matrix91& e, Matrix91& r)
{
    ...
    Matrix33 R = svd.matrixU() * Matrix31({1, 1, detUV}).asDiagonal() * svd.matrixV().transpose();
    ...
    const double& det_threshold = 1e-8)
{
    ...
    double a = Q(0, 0), b = Q(1, 0), d = Q(1, 1),
  ...
  Matrix91& delta )
{
  ...
  Matrix31 y = -Winv * NtOmega * ( delta + r );
...
RowAndNullSpace(const Matrix91& r, 
