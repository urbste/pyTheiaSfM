...
ViewGraph FilterViewPairsFromOrientationWrapper(
  ...
  FilterViewPairsFromOrientation(
      orientations, max_relative_rotation_difference_degrees, &view_pairs);
  return view_pairs;
}
...
LocalizeViewToReconstructionWrapper(
  ...
  const bool success = LocalizeViewToReconstruction(
      view_to_localize, options, &reconstruction, &summary);
...
std::tuple<int, Reconstruction> SetOutlierTracksToUnestimatedAllWrapper(
  ...
  Reconstruction reconstruction;
  int num_features_rm =
      SetOutlierTracksToUnestimated(max_inlier_reprojection_error,
                                    min_triangulation_angle_degrees,
                                    &reconstruction);
  return std::make_tuple(num_features_rm, reconstruction);
}

// std::tuple<bool, FloatImage> UndistortImageWrapper(const Camera&
// distorted_camera,
//                    const FloatImage& distorted_image,
//                    const Camera& undistorted_camera){
//    FloatImage undistorted_image;
//    const bool success = UndistortImage(distorted_camera, distorted_image,
//    undistorted_camera,&undistorted_image); return std::make_tuple(success,
//    undistorted_image);
//}

// std::tuple<bool, Camera> UndistortCameraWrapper(const Camera&
// distorted_camera){
//    Camera undistorted_camera;
//    const bool success = UndistortCamera(distorted_camera,
//    &undistorted_camera); return std::make_tuple(success, undistorted_camera);
//}
